...
def get_cls_results_round(det_results, annotations, class_id):
    """Get det results and gt information of a certain class.
    ...
    for i, det in enumerate(cls_dets):
        det[:, -1] = det[:, -1].round(1)
        # det[:, -1] = np.where(det[:, -1]>0.9, 0.9, det[:, -1])
        cls_dets[i] = det 
...
def eval_rbbox_map(det_results,
    ...
    Returns:
        tuple: (mAP, [dict, dict, ...])
    """
    assert len(det_results) == len(annotations), f"det_result len: {len(det_results)}\nann len: {len(annotations)}"
    ...
    for i in range(num_classes):
        ...
        # round_dets = cls_dets.copy()
        # round_dets[:, -1] =round_dets[:, -1].round(1) 
        ...
        # tp_11 = []
        # fp_11 = []
        # for cls_thr in np.linspace(0.9, 0, 10):
        #     thr_mask = cls_dets[:, -1] > cls_thr
        #     tp_11.append(tp[:, thr_mask].sum())
        #     fp_11.append(fp[:, thr_mask].sum())
        # tp = np.array([tp_11])
        # fp = np.array([fp_11])
        # sort_inds = np.argsort(-round_dets[:, -1])
        ...
        # prec = np.array([float("%.3f"%i) for i in precisions])
        # rec = np.array([float("%.3f"%i) for i in recalls])
    pool.close()

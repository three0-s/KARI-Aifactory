...
import numpy as np
import torch
from mmcv.ops import nms_rotated, soft_nms
from mmdet.datasets.custom import CustomDataset

from mmrotate.core import eval_rbbox_map, obb2poly_np, poly2obb_np, obb2hbb
...
class DOTADataset(CustomDataset):
    ...
    def merge_det(self, results, nproc=4, iou_thr=0.5):
    ...
    def format_results(self, results, submission_dir=None, nproc=4, merge_iou_thr=0.5, **kwargs):
        ...
        if submission_dir is None:
            submission_dir = str(tempfile.TemporaryDirectory())
        ...
        try:
            ...
            id_list, dets_list = self.merge_det(results, nproc, iou_thr=merge_iou_thr)
...
def _merge_func(info, CLASSES, iou_thr):
    ...
    for i in range(len(CLASSES)):
        if len(dets[labels == i]) == 0:
            ...
            # nms_dets, keep_inds = soft_nms(obb2hbb(cls_dets[:, :5])[:, :4].float(), cls_dets[:, -1].float(), iou_thr, min_score=0.1)
            # nms_dets = cls_dets[keep_inds, ...]
